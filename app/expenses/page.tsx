...
export default async function ExpensesPage() {
  ...
  const [{ data: phasesData }, { data: catsData }, { data: entriesData }, { data: incomesData }] = await Promise.all([
    supabase.from("phases").select("*").order("start_date", { ascending: false }),
    supabase.from("categories").select("*").eq("kind", "expense").order("name", { ascending: true }),
    supabase
      .from("expenses")
      .select("*, category:categories(id, name), phase:phases(id, name, end_date)")
      .order("date", { ascending: false }),
    supabase.from("incomes").select("amount, date, phase_id"),
  ]);

  const phases = (phasesData ?? []) as Phase[];
  const categories = (catsData ?? []) as Category[];
  const entries = (entriesData ?? []) as EntryWithJoins[];
  const incomes = (incomesData ?? []) as { amount: number; date: string; phase_id: string }[];
  ...
  // Per-category breakdown for current phase: past monthly average vs. current month
  ...
  // Avg monthly income (past months only, same divisor) — for the "% of income" column.
  const phaseIncomePast = incomes
    .filter((r) => r.phase_id === currentPhase.id && r.date < monthStart)
    .reduce((a, r) => a + Number(r.amount), 0);
  const avgMonthlyIncome = phaseIncomePast / monthsForAvg;
  const pctOfIncome = (n: number) =>
    avgMonthlyIncome > 0 ? `${((n / avgMonthlyIncome) * 100).toFixed(1)}%` : "—";
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="mb-3 text-sm font-medium text-zinc-500">Add expense</h2>
        <AddExpenseForm categories={categories} phaseStart={currentPhase.start_date} />
      </section>
      ...
      {breakdown.length > 0 && (
        <section className="overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <ul className="divide-y divide-zinc-200 dark:divide-zinc-800">
            <li className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-4 px-4 py-2 text-[10px] font-medium uppercase tracking-wide text-zinc-400">
              <span>Category</span>
            ...
            {breakdown.map((s) => (
              <li key={s.name} className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-4 px-4 py-2 text-sm">
                ...
                <span className="w-20 text-right tabular-nums text-zinc-500">{pctOfIncome(s.avg)}</span>
                <span className="w-24 text-right tabular-nums">{fmtINR(s.current)}</span>
              </li>
            ))}
            <li className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-4 bg-zinc-50 px-4 py-2 text-sm font-medium dark:bg-zinc-900/40">
              <span>Total</span>
              <span className="w-24 text-right tabular-nums">{fmtINR(breakdownTotalAvg)}</span>
              <span className="w-20 text-right tabular-nums text-zinc-500">{pctOfIncome(breakdownTotalAvg)}</span>
